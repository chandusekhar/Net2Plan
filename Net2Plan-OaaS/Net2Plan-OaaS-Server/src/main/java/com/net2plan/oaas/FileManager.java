...
public class FileManager {
    ...
    public static JSONObject writePersistenceFileCatalog (String catalog)
    {
        ...
        try  {
            ...
            FileUtils.writeByteArrayToFile(new File(ServerUtils.TOMCAT_FILES_DIR+ File.separator + "catalog.n2p"), catalog.getBytes());
            response.put("status",new JSONValue("OK"));
            response.put("content",new JSONValue("No content for this function"));
        }catch(Exception ex){
            response.put("status", new JSONValue("ERROR"));
    ...
    public static JSONObject readPersistenceFileCatalog ()
    {
        JSONObject response = new JSONObject();
