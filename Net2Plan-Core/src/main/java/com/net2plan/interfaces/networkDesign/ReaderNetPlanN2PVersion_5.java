...
class ReaderNetPlanN2PVersion_5 implements IReaderNetPlan //extends NetPlanFormat_v3
{
    ...
    {
        if (ErrorHandling.isDebugEnabled()) ProfileUtils.printTime("", -1);
...
//		System.out.println ("End ReaderNetPlan_v5: " + netPlan + " ----------- ");
    ...
    protected void parseNetwork(NetPlan netPlan) throws XMLStreamException
    {
        ...
        while (true)
        {
            try
            {
                netPlan.addGlobalPlanningDomain(getString("planningDomain_" + (netPlan.getGlobalPlanningDomains().size())));
            } catch (Exception e)
            {
                break;
            }
...
//						if (!netPlan.hasReadLayerZero && netPlan.layers.size() > 1) netPlan.removeNetworkLayer (netPlan.layers.get(0));
    ...
    private void parseNode(NetPlan netPlan) throws XMLStreamException
    {
        ...
        if (nodeId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        ...
        try
        {
            population = getDouble("population");
        } catch (Exception e)
        {
        }
        ...
        while (true)
        {
            try
            {
                planningDomains.add(getString("planningDomain_" + (planningDomains.size())));
            } catch (Exception e)
            {
                break;
            }
        ...
        for (long layerId : getListLong("layersWithIconsDefined"))
        {
            ...
            if (iconsThisLayerSoFar == null)
            {
                iconsThisLayerSoFar = new LinkedList<>();
                nodeAndLayerToIconURLMap.put(layerId, iconsThisLayerSoFar);
            }
            URL url = null;
            try
            {
    ...
    private void parseDemand(NetPlan netPlan, long layerId) throws XMLStreamException
    {
        ...
        if (demandId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        ...
        try
        {
            recoveryType = Demand.IntendedRecoveryType.valueOf(getString("intendedRecoveryType"));
        } catch (XMLStreamException e)
        {
            recoveryType = Demand.IntendedRecoveryType.NOTSPECIFIED;
        } catch (Exception e)
        {
            recoveryType = Demand.IntendedRecoveryType.UNKNOWNTYPE;
        }
        ...
        while (xmlStreamReader.hasNext() && !finalElementRead)
        {
            ...
            switch (xmlStreamReader.getEventType())
            {
                ...
                case XMLEvent.END_ELEMENT:
                    String endElementName = xmlStreamReader.getName().toString();
                    if (endElementName.equals("demand"))
                    {
                        finalElementRead = true;
                        break;
                    }
            }
        }
    ...
    private void parseRoute(NetPlan netPlan, long layerId) throws XMLStreamException
    {
        ...
        if (routeId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        ...
        try
        {
            initialStatePath = getLinkAndResorceListFromIds(netPlan, getListLong("initialStatePath"));
        } catch (Exception e)
        {
    ...
    private void parseLink(NetPlan netPlan, long layerId) throws XMLStreamException
    {
        ...
        if (linkId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
    ...
    private void parseSRG(NetPlan netPlan) throws XMLStreamException
    {
        ...
        if (srgId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
    ...
    private void parseResource(NetPlan netPlan) throws XMLStreamException
    {
        ...
        if (resId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        final long hostNodeId = getLong("hostNodeId");
        if (netPlan.getNodeFromId(hostNodeId) == null)
            throw new Net2PlanException("Could not find the hot node of a resource when reading");
        ...
        try
        {
            urlIcon = new URL(getString("urlIcon"));
        } catch (Exception e)
        {
        }
    ...
    private void parseLayer(NetPlan netPlan) throws XMLStreamException
    {
        ...
        if (layerId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        ...
        try
        {
            defaultNodeIconURL = new URL(getString("defaultNodeIconURL"));
        } catch (Exception e)
        {
        }
                ...
                case XMLEvent.END_ELEMENT:
                    ...
                    if (endElementName.equals("sourceRouting"))
                    {
                        ...
                        for (Entry<Route, List<Long>> entry : this.backupRouteIdsMap.entrySet())
                        {
                            ...
                            for (long backupId : entry.getValue())
                                primary.addBackupRoute(netPlan.getRouteFromId(backupId));
                        }
                        return;
                    }
                    break;
    ...
    private void parseMulticastTree(NetPlan netPlan, long layerId) throws XMLStreamException
    {
        ...
        if (treeId >= netPlan.nextElementId.toLong())
            throw new Net2PlanException("A network element has an id higher than the nextElementId");
        final long demandId = getLong("demandId");
        ...
        double carriedTrafficIfNotFailing = carriedTraffic;
        try
        {
            carriedTrafficIfNotFailing = getDouble("carriedTrafficIfNotFailing");
        } catch (Exception e)
        {
        }
        double occupiedLinkCapacityIfNotFailing = occupiedCapacity;
        try
        {
            occupiedLinkCapacityIfNotFailing = getDouble("occupiedLinkCapacityIfNotFailing");
        } catch (Exception e)
        {
        }
    ...
    {
        return Boolean.parseBoolean(xmlStreamReader.getAttributeValue(xmlStreamReader.getAttributeIndex(null, name)));
    }
    ...
    {
        return xmlStreamReader.getAttributeAsLong(xmlStreamReader.getAttributeIndex(null, name));
    }

    private List<Long> getListLong(String name) throws XMLStreamException
    {
        return LongUtils.toList(xmlStreamReader.getAttributeAsLongArray(xmlStreamReader.getAttributeIndex(null, name)));
    }

    private List<Double> getListDouble(String name) throws XMLStreamException
    {
        return DoubleUtils.toList(xmlStreamReader.getAttributeAsDoubleArray(xmlStreamReader.getAttributeIndex(null, name)));
    }

    private double getDouble(String name) throws XMLStreamException
    {
        return xmlStreamReader.getAttributeAsDouble(xmlStreamReader.getAttributeIndex(null, name));
    }

    private static Set<Link> getLinkSetFromIds(NetPlan np, Collection<Long> ids)
    {
        Set<Link> res = new HashSet<Link>();
        for (long id : ids) res.add(np.getLinkFromId(id));
        return res;
    }
    ...
    private static Set<Node> getNodeSetFromIds(NetPlan np, Collection<Long> ids)
    {
        ...
        for (long id : ids) res.add(np.getNodeFromId(id));
        return res;
    }
    ...
    private static Map<Resource, Double> getResourceOccupationMap(NetPlan np, List<Double> resAndOccList)
    {
        ...
        while (it.hasNext())
        {
            ...
            final Resource r = np.getResourceFromId(id);
            if (r == null) throw new Net2PlanException("Unknown resource id");
            res.put(r, val);
        }
        return res;
    }
